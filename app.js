...
const STRINGS = {
  en: {
    ...
    orderConfirmedTitle: "Payment successful",
    orderConfirmedBody: "Thanks for your order — a confirmation has been sent to your email.",
    checkoutCancelledTitle: "Checkout cancelled",
    checkoutCancelledBody: "Your cart is still saved, whenever you're ready.",
...
function cartIconHTML() {
  return `
    <button class="cart-icon-btn" onclick="goCart()" aria-label="${t("cart")}">
...
function renderCart() {
  document.getElementById("topbar-left").innerHTML = `
    <button class="back-link" onclick="goHome()">
      <i class="ti ti-arrow-left" aria-hidden="true"></i> ${t("backToHome")}
    </button>`;
  ...
  if (items.length === 0) {
    document.getElementById("app").innerHTML = `
      <section class="product">
        <h1 style="margin-top: 24px;">${t("yourCart")}</h1>
        <div class="empty-state">${t("emptyCart")}</div>
        <div style="text-align:center; margin-top: 16px;">
          <button class="btn btn-primary" onclick="goHome()">${t("continueShopping")}</button>
        </div>
      </section>`;
    return;
  }
  ...
  const rows = items
    .map(
      (item) => `
      <div class="cart-row">
        <div class="cart-row-media"><i class="ti ${formIcon(item.form)}" aria-hidden="true"></i></div>
        <div class="cart-row-info">
          <h3>${item.name}</h3>
          <p>${item.strength || "\u2014"} &nbsp;\u00b7&nbsp; ${item.form}</p>
        </div>
        <div class="qty-control">
          <button onclick="cartSetQty('${item.id}', ${item.qty - 1}); render()" aria-label="Decrease quantity">\u2212</button>
          <span>${item.qty}</span>
          <button onclick="cartSetQty('${item.id}', ${item.qty + 1}); render()" aria-label="Increase quantity">+</button>
        </div>
        <div class="cart-row-price">${money(item.price * item.qty)}</div>
        <button class="cart-remove" onclick="cartRemove('${item.id}'); render()" aria-label="${t("remove")}">
    ...
    <section class="product">
      ...
      <div class="cart-summary">
        <div class="cart-total-row">
          <span>${t("total")}</span>
          <span class="price-value">${money(cartTotal())}</span>
        </div>
        <button class="btn btn-primary btn-add" id="checkout-btn" onclick="checkout()">${t("checkout")}</button>
      </div>
    </section>`;
}
...
function renderCheckoutResult(kind) {
  document.getElementById("topbar-left").innerHTML = "";
  document.getElementById("topbar-right").innerHTML = renderLangSwitch();

  const success = kind === "success";
  if (success) cartClear();

  document.getElementById("app").innerHTML = `
    <section class="welcome">
      <div class="info-icon" style="margin: 0 auto 20px; width:56px; height:56px; font-size:26px;">
        <i class="ti ${success ? "ti-circle-check" : "ti-circle-x"}" aria-hidden="true"></i>
      </div>
      <h1 style="font-size:32px;">${success ? t("orderConfirmedTitle") : t("checkoutCancelledTitle")}</h1>
      <p class="tagline">${success ? t("orderConfirmedBody") : t("checkoutCancelledBody")}</p>
...
function renderHome() {
  document.getElementById("topbar-left").innerHTML = "";
  document.getElementById("topbar-right").innerHTML = cartIconHTML() + renderLangSwitch();
...
function render() {
  ...
  } else if (hash === "#checkout-cancelled") {
    renderCheckoutResult("cancelled");
  } else {
    renderHome();
  }
  updateCartBadge();
